...
const TwistedSearch: React.FC<TwistedSearchProps> = ({ onSelectTwisted }) => {
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-2">Twisted Strategy Search</h2>
        <p className="text-gray-600">Search for specific survival strategies and scenarios</p>
      </div>
      ...
      <div className="mb-6 grid grid-cols-1 md:grid-cols-3 gap-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Strategy Category</label>
          <select
              ...
              <div key={index} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h4 className="font-semibold text-gray-800">{result.twisted.name}</h4>
                      <p className="text-sm text-gray-600">{result.strategy.title}</p>
                    </div>
                  </div>
                  <div className="flex space-x-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      result.strategy.difficulty === 'expert' ? 'bg-purple-100 text-purple-600' :
                      result.strategy.difficulty === 'hard' ? 'bg-red-100 text-red-600' :
                      result.strategy.difficulty === 'medium' ? 'bg-yellow-100 text-yellow-600' :
                      'bg-green-100 text-green-600'
                    }`}>
                      {result.strategy.difficulty}
                    </span>
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      result.strategy.effectiveness === 'critical' ? 'bg-red-100 text-red-600' :
                      result.strategy.effectiveness === 'high' ? 'bg-green-100 text-green-600' :
                      result.strategy.effectiveness === 'medium' ? 'bg-yellow-100 text-yellow-600' :
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filteredTwisted.map(entity => (
            <div key={entity.id} className="bg-white border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer">
              <div className="flex items-center space-x-3 mb-3">
                ...
                <div>
                  ...
                  <div className="flex space-x-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      entity.dangerLevel === 'extreme' ? 'bg-red-100 text-red-600' :
                      entity.dangerLevel === 'high' ? 'bg-orange-100 text-orange-600' :
                      entity.dangerLevel === 'medium' ? 'bg-yellow-100 text-yellow-600' :
                      'bg-green-100 text-green-600'
                    }`}>
                      {entity.dangerLevel}
                    </span>
